...
public class RoadClosures {
    private static final String TAG = "roadClosuresClass";
    ...
    public ArrayList<String> getClosureDate() {
        ...
        for (int i = 1; i < closures.get(0).getElementsByTag("table").get(0).getElementsByTag("tr").size(); i++) {
            ...
            //  Log.i(TAG, closures.get(0).getElementsByTag("table").get(0).getElementsByTag("tr").get(i).getElementsByTag("td").get(1).text());
        }
        return dates;
    }
    ...
    public boolean isClosureScheduledForToday() {
        ...
        for (int i = 0; i < getClosureDate().size(); i++) {
            ...
            //    timeOfClosure = closures.get(0).getElementsByTag("table").get(0).getElementsByTag("tr").get(k).getElementsByTag("td").get(2).text();
            if (i < getClosureDate().size() - 1)
                k++;
            if (isClosure) break;
        }
        return isClosure;
